...
app.post('/api/clients', async (req, res) => {
  ...
  try {
    ...
    // Insérer les détails personnels
    ...
    // Insérer les détails de l'entreprise (sans Annual Revenue et Total Funding)
    ...
    // Insérer les détails géographiques
    ...
    // Insérer les détails de revenus
    const revenueResult = await client.query(
      `INSERT INTO companyrevenue (
        "companyid", "Latest Funding", "Latest Funding Amount"
      )
      VALUES ($1, $2, $3)
      RETURNING *`,
      [
...
app.post('/api/ressources/update', async (req, res) => {
  if (!req.body) {
    console.error("Request body is undefined");
    return res.status(400).json({ error: "Request body is required." });
  }

  const { id, personalDetails, companyDetails, geoDetails, revenueDetails, socialDetails } = req.body;

  if (!id) {
    console.error("ID is missing in the request body");
    return res.status(400).json({ error: "ID is required to update the data." });
  }
  ...
  try {
    ...
    if (personalDetails) {
      await client.query(
        `UPDATE personaldetails
         SET "First Name" = $1, "Last Name" = $2, "title" = $3, "seniority" = $4, "departments" = $5, 
             "mobilePhone" = $6, "email" = $7, "EmailStatus" = $8
         WHERE personalid = $9`,
        [
    ...
    if (companyDetails) {
      await client.query(
        `UPDATE companydetails
         SET "company" = $1, "Email" = $2, "Phone" = $3, "employees" = $4, "industry" = $5, 
             "SEO Description" = $6 
         WHERE personalid = $7`,
        [
          ...
          parseInt(companyDetails.employees, 10) || null,
    ...
    if (revenueDetails) {
      await client.query(
        `UPDATE companyrevenue
         SET "Latest Funding" = $1, "Latest Funding Amount" = $2
         WHERE companyid = (SELECT companyid FROM companydetails WHERE personalid = $3)`,
        [
          revenueDetails.latestFunding || null,
          revenueDetails.latestFundingAmount || '',
          id,
        ]
      );
    }
    ...
    if (socialDetails) {
      await client.query(
        `UPDATE socialdetails
         SET "Company Linkedin Url" = $1, "Facebook Url" = $2, "Twitter Url" = $3
