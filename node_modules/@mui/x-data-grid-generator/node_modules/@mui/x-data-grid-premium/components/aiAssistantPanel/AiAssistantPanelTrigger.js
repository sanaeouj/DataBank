 ...
 */
const AiAssistantPanelTrigger = exports.AiAssistantPanelTrigger = (0, _forwardRef.forwardRef)(function AiAssistantPanelTrigger(props, ref) {
  const {
      render,
      className,
      onClick,
  ...
  const panelState = (0, _xDataGridPro.useGridSelector)(apiRef, _xDataGridPro.gridPreferencePanelStateSelector);
  const open = panelState.open && panelState.openedPanelValue === _xDataGridPro.GridPreferencePanelsValue.aiAssistant;
  const state = {
    open
  };
  ...
  const handleClick = event => {
    if (open) {
      ...
      apiRef.current.showPreferences(_xDataGridPro.GridPreferencePanelsValue.aiAssistant, panelId, buttonId);
    }
    onClick?.(event);
  };
